...
import { CreateServiceGQL, DeleteServiceGQL, NewServiceQuery, ServiceOptionIcon, ServicePaymentMethod, SetOptionsOnServiceGQL, SetRegionsOnServiceGQL, UpdateServiceGQL, ViewServiceQuery } from '@ridy/admin-panel/generated/graphql';
...
export class ManagementServicesViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.data.subscribe(data => {
      ...
      service.regions = service.regions.map((region: { id: string }) => region.id) as unknown as any;
      service.options = service.options.map((region: { id: string }) => region.id) as unknown as any;
  ...
  async onSubmit() {
    ...
    try {
      ...
      await firstValueFrom(this.setOptionsGQL.mutate({ id: resId, relationIds: options }));
  ...
  onItemChecked(id: string, checked: boolean): void {
    if (checked) {
      ...
      this.form.value.options = this.form.value.options.filter((_region: string) => _region != id);
    }
  }
}
